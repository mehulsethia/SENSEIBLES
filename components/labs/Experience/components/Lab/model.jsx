...
const Model = () => {
  ...
  const colorA = pages[0]?.color ?? fallbackColor;
  const colorB = pages[1]?.color ?? colorA;

  const group = useRef();
  const { scene } = useGLTF(modelUrl);
  const { camera } = useThree();

  const colorStart = "#b9bbc2";
  const colorEnd = "#e8e8e8";
  ...
  const floorMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uColor1: new THREE.Uniform(new THREE.Color(colorA)),
        uColor2: new THREE.Uniform(new THREE.Color(colorB)),
  ...
  }, [bakedFloor, bakedPowerTexture, colorA, colorB]);
  ...
  }, [bakedPowerTexture, bakedProps, colorA, colorB]);
  ...
  }, [colorEnd, colorStart, noiseTexture]);
  ...
  }, [bgMaterial, faceMaterial, floorMaterial, noisePlaneMaterial, pressMaterial, scene, smileMaterial]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = useStats.subscribe(
      (state) => state.index,
      (value, prevValue) => {
        const previousColor = pages[prevValue]?.color ?? colorA;
        const currentColor = pages[value]?.color ?? colorB;

        floorMaterial.uniforms.uColor1.value = new THREE.Color(previousColor);
        floorMaterial.uniforms.uColor2.value = new THREE.Color(currentColor);
